...
int main (int argc, char **argv)
{
	...
	//parse sthe command line arguments
	parse_cmdline(argc, argv);
	
	// opene the /dev/mem device
	devmem_filedesc = open ("/dev/mem", 0_RDWR | 0_SYNC);
		...
		(void)
{
	...
	if((ALT_LWFPGASLVS_OFST + BLINKER_0_BASE) & (sysconf(_SC_PAGE_SIZE) - 1))
	{
		printf("ERROR: blinker base 0x%08X is not page aligned", 
			   ALT_LWPFGASLVS_OFST + BLINKER_0_BASE);
...
void do_write_config (void *blinker_driver_map)
{
	volatile unsigned char *blinker_config_reg = blinker_driver_map + BLINKER_REG_OFFSET;
